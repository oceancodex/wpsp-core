...
class MakeRoleCommand extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		...
		if (!$name) {
			...
			if (empty($name)) {
				$this->writeln($output, 'Missing name for the role. Please try again.');
		...
		// Check exist.
		$exist = FileSystem::exists($this->mainPath . '/app/Components/Roles/' . $nameSlugify . '.php');
		if ($exist) {
			$this->writeln($output, '[ERROR] Role: "' . $name . '" already exists! Please try again.');
		...
		FileSystem::put($this->mainPath . '/app/Components/Roles/' . $nameSlugify . '.php', $content);
