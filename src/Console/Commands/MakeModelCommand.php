...
class MakeModelCommand extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		...
		if (!$name) {
			...
			if (empty($name)) {
				$this->writeln($output, 'Missing name for the model. Please try again.');
		...
		if ($exist) {
			$this->writeln($output, '[ERROR] Model: "' . $name . '" already exists! Please try again.');
			return Command::FAILURE;
		}

		$table   = $table ?? $input->getOption('table') ?: 'my_custom_table';
		...
		// Output message.
		$this->writeln($output, '<green>Created new model: "' . $name . '"</green>');
