...
abstract class BasePluginColumn extends BaseInstances {
	...
	public function init($column_name = null) {
		...
		if ($column_name) {
			 ...
			 */
			add_filter('manage_plugins_columns', function($columns) use ($column_name) {
				$new_columns = [];
				$inserted = false;
			 ...
			 * The column content.
			 */
			add_filter('manage_plugins_custom_column', function($columnName, $pluginFile, $pluginData) use ($requestPath, $column_name) {
			...
			/**
			 * Sortable column. (X)\
			 * Plugin list table không có sortable.
			 */
//			if ($this->sortable) {
