 ...
 */
abstract class BaseCommentColumn extends BaseInstances {
	...
	public function init($column_name = null) {
		...
		if ($column_name) {
			 ...
			 */
			add_filter('manage_edit-comments_columns', function($columns) use ($column_name) {
				$new_columns = [];
				$inserted = false;
			 ...
			 * The column content.
			 */
			add_filter('manage_comments_custom_column', function($columnName, $commentId) use ($requestPath, $column_name) {
			...
			if (method_exists($this, 'sort')) {
				add_action('pre_get_comments', [$this, 'sort'], 9999999999);
			}
